...
-- Trigger: new user -> profile, role, starter coins
...
-- RPC: place_bet (deduct coins atomically, update pool)
...
BEGIN
  ...
  IF _gang_id NOT IN (_match.gang_a_id, _match.gang_b_id) THEN RAISE EXCEPTION 'Invalid gang for this match'; END IF;
...
BEGIN
  ...
  -- Edge case: no bets on winning side -> refund losing side
